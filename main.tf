...
resource "yandex_vpc_security_group" "lamp_sg" {
  ...
  # Разрешить трафик от балансировщика (внутренняя сеть)
  ingress {
    protocol       = "TCP"
  ...
  # Разрешить трафик от ALB (health checks)
  ingress {
    protocol       = "TCP"
...
# Создание сервисного аккаунта для Instance Group и Object Storage
...
# Назначение роли редактора (широкие права)
